import { Briefcase, Construction } from "lucide-react";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

const JobRolesPage = () => {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">
          Job Role Eligibility
        </h2>
        <p className="text-muted-foreground mt-2">
          Discover job roles that match your resume and skillset
        </p>
      </div>

      <Card className="border-dashed">
        <CardHeader>
          <div className="flex items-center gap-2">
            <Construction className="h-5 w-5 text-amber-500" />
            <CardTitle>Feature Under Development</CardTitle>
          </div>
          <CardDescription>
            This feature is currently being developed and will be available
            soon.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <Briefcase className="h-16 w-16 text-muted-foreground/50 mb-4" />
            <h3 className="text-xl font-semibold mb-2">Coming Soon!</h3>
            <p className="text-muted-foreground max-w-md mb-4">
              Analyze your resume to find job roles you're eligible for. Get
              insights on which positions match your experience and skills.
            </p>
            <div className="flex gap-2 flex-wrap justify-center">
              <Badge variant="secondary">Role Matching</Badge>
              <Badge variant="secondary">Eligibility Score</Badge>
              <Badge variant="secondary">Career Suggestions</Badge>
              <Badge variant="secondary">Market Insights</Badge>
            </div>
          </div>
        </CardContent>
      </Card>
    </div>
  );
};

export default JobRolesPage;
